...
ring_buffer_t* ring_buffer_init( size_t queue_size, size_t obj_size )
{
	...
	uint32_t size_new = to_the_power_of_2( size_q ) * 2; // herc enlarge

	uorbc_platform_log( "queue_size(%d)*obj_size(%d):%d size_new:%d\r\n", queue_size, obj_size, size_q, size_new );
	...
	if( NULL != p_ring ) {
		...
		uorbc_platform_log( "buffer_size:%d buffer_mask:%d\r\n", p_ring->buffer_size, p_ring->buffer_mask );
		return p_ring;
	}
...
//	uorbc_platform_log("peek_pos:%d len:%d", pos, len);
	...
	while( cnt < len ) {
		*data_ptr = buffer->buffer[peek_pos];
		//uorbc_platform_log("ring_buffer_just_peek:  data_ptr = %d , buffer->buffer[%d] = %d \r\n", *data_ptr, peek_pos, buffer->buffer[peek_pos]);
